    ...
    var calendarVue = new Vue({
        ...
        methods: {
            initData: function(current) {
                var data,dataP,dataN;
                this.days = [];

                if(current) {
                    data = new Date(current);
                    // dataP = new Data(this.formatDate(data.getFullYear(), data.getMonth(), data.getDate()));
                    // dataN = new Data(this.formatDate(data.getFullYear(), data.getMonth()+2, data.getDate()))
                }else {
                    var now = new Date();
            ...
            pickPre: function() {
                ...
                var data = new Date(this.formatDate(temyear, temmonth+1, 1));
                day = (new Date(this.formatDate(temyear, temmonth+1, this.date))).getDay();
                this.day = week[day];
                this.initData(data);
            },
            pickNext: function() {
